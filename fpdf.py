...
class FPDF(object):
    ...
    def __init__(self, orientation='P',unit='mm',format='A4',pw=0,ph=0):
        # Some checks
        self._dochecks()
        ...
        if(isinstance(format,str)):
            format=format.lower()
            if(format=='4a0'):
                format=(1682/mmToPt,2378/mmToPt)
            elif(format=='2a0'):
                format=(1189/mmToPt,1682/mmToPt)
            elif(format=='a0'):
                format=(841/mmToPt,1189/mmToPt)
            elif(format=='a1'):
                format=(594/mmToPt,841/mmToPt)
            elif(format=='a2'):
                format=(420/mmToPt,594/mmToPt)
            elif(format=='a3'):
    ...
    def normalize_text(self, txt):
        ...
        if self.unifontsubset and isinstance(txt, str):
            txt = txt.decode('utf8')
        elif not self.unifontsubset and not PY3K:
...
#            self.error("Don\'t alter the locale before including class file");
    ...
    def _parsejpg(self, filename):
        ...
        try:
            ...
            self.error('Missing or incorrect image file: %s. ' % (filename))
        else:
            a = im.size
    ...
    def _parsegif(self, filename):
        ...
        try:
            ...
            self.error('Missing or incorrect image file: %s. error: %s' % (filename))
        else:
            # Use temporary file
    ...
    def interleaved2of5(self, txt, x, y, w=1.0, h=10.0):
        ...
        for i in range(0, len(code), 2):
            ...
            if not char_bar in bar_char.keys():
                raise RuntimeError ('CaractŽr "%s" inv‡lido para el c—digo de barras I25: ' % char_bar)
            if not char_space in bar_char.keys():
                raise RuntimeError ('CaractŽr "%s" inv‡lido para el c—digo de barras I25: ' % char_space)
    ...
    def code39(self, txt, x, y, w=1.5, h=5.0):
        ...
        for i in range (0, len(code), 2):
            ...
            if not char_bar in bar_char.keys():
                raise RuntimeError ('Caracter "%s" inv‡lido para el c—digo de barras' % char_bar)
